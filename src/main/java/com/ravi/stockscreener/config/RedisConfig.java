...
//        // try to initialize but don't fail hard if unreachable
//        try {
//            factory.afterPropertiesSet();
//            log.info("Configured LettuceConnectionFactory (host={}, port={}, cmdTimeout={}s)", redisHost, redisPort, COMMAND_TIMEOUT.getSeconds());
//        } catch (Exception ex) {
//            log.warn("Redis not reachable during factory init (continuing without hard-fail): {}", ex.toString());
//            // returning factory — subsequent uses will throw if they try to use it,
//            // but app won't crash at startup just because Redis is down during build.
//        }
//        return factory;
//    }
...
//    }
    ...
    @Bean
    @ConditionalOnProperty(name = "app.redis.enabled", havingValue = "true", matchIfMissing = true)
    public ReactiveStringRedisTemplate reactiveStringRedisTemplate(ReactiveRedisConnectionFactory lettuceConnectionFactory) {
